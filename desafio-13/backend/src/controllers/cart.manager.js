...
class CartManager {
  ...
  async addProductIntoCart(idCarrito, idProducto, user) {
    ...
    /*
    await carrito.save();
    return this.getCartById(idCarrito)
    */
  };
  ...
  async deleteProductIntoCart(idCarrito, idProducto) {
    ...
    let productoEnCarrito = carrito.products.find(x => x.product._id.equals(idProducto));

    if (productoEnCarrito && productoEnCarrito.quantity > 1) {
      productoEnCarrito.quantity--;
      
    } else if(productoEnCarrito.quantity == 1) {      
      const products = carrito.products.find((item) => item.product._id !== idProducto);
  ...
  async finalizePurchase(idCarrito, user) {
    ...
    const productosConStock = carrito.products.filter(x => {      
      return x.product.stock >= x.quantity
      //return productManager.getStock(x.product, x.quantity)
    });

    const productosSinStock = carrito.products.filter(x => {      
      return x.product.stock < x.quantity
    });  
    
    for (const x of productosConStock) {
      await productManager.updateProduct(x.product._id, { stock: (x.product.stock - x.quantity) })      
    }
